...
ok("at 9am only the full stores are lit",
   ids(bagelKitchens) === "wilshire,larchmont,glendon,ventura,pasadena,fullerton,longbeach,torrance,sanclemente,gardengrove", ids(bagelKitchens));
...
ok("a sandwich narrows to the full stores",
   ids(withSandwich) === "wilshire,larchmont,glendon,ventura,pasadena,fullerton,longbeach,torrance,sanclemente,gardengrove", ids(withSandwich));
...
try {
  ...
  ok("those stay with the counters that can, every full store",
     sandwiches.map((s) => s.id).sort().join(",") === "fullerton,gardengrove,glendon,larchmont,longbeach,pasadena,sanclemente,torrance,ventura,wilshire",
